...
The canonical v0.6 selector is event-aware: it keeps sparse representatives of
...
per-path cap and do not replace the canonical semantic selection. The default
zero target count adds no scored-POI floor in event-aware mode.
