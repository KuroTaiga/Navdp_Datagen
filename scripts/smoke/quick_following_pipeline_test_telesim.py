...
def _parse_args() -> argparse.Namespace:
    ...
    ap.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=True,
        help="If no matching source manifest is found, generate one via scripts/actions/generate_assignment_manifest.sh (default: on).",
    )
    ap.add_argument(
        "--actor-root",
        type=Path,
    ...
    if source_manifest is None or not source_manifest.is_file():
        ...
        if ban_list is None:
            ...
            if candidate.is_file():
                ban_list = candidate
        gen_script = REPO_ROOT / "scripts" / "actions" / "generate_assignment_manifest.sh"
