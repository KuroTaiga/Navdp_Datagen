...
_storage_repo_root="$(cd "${_storage_dir}/../.." && pwd)"
_storage_config_loaded="${_storage_config_loaded:-false}"

if ! $_storage_config_loaded; then
  if [ -z "${STORAGE_TARGETS_CONFIG_FILE:-}" ]; then
    if [ -f "${_storage_dir}/storage_remote_config.sh" ]; then
      STORAGE_TARGETS_CONFIG_FILE="${_storage_dir}/storage_remote_config.sh"
    else
      STORAGE_TARGETS_CONFIG_FILE="${_storage_repo_root}/storage_remote_config.sh"
    fi
  fi
