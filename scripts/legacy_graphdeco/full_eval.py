...
def _python_cmd(script: Path, args: str) -> str:
    return f"{shlex.quote(sys.executable)} {shlex.quote(str(script))} {args}"
...
if not args.skip_training:
    ...
    for scene in mipnerf360_outdoor_scenes:
        ...
        os.system(_python_cmd(TRAIN_SCRIPT, "-s " + source + " -i images_4 -m " + args.output_path + "/" + scene + common_args))
    for scene in mipnerf360_indoor_scenes:
        source = args.mipnerf360 + "/" + scene
        os.system(_python_cmd(TRAIN_SCRIPT, "-s " + source + " -i images_2 -m " + args.output_path + "/" + scene + common_args))
    m360_timing = (time.time() - start_time)/60.0

    start_time = time.time()
    for scene in tanks_and_temples_scenes:
        source = args.tanksandtemples + "/" + scene
        os.system(_python_cmd(TRAIN_SCRIPT, "-s " + source + " -m " + args.output_path + "/" + scene + common_args))
...
if not args.skip_rendering:
    ...
    for scene, source in zip(all_scenes, all_sources):
        os.system(_python_cmd(RENDER_SCRIPT, "--iteration 7000 -s " + source + " -m " + args.output_path + "/" + scene + common_args))
        os.system(_python_cmd(RENDER_SCRIPT, "--iteration 30000 -s " + source + " -m " + args.output_path + "/" + scene + common_args))
...
if not args.skip_metrics:
    ...
    os.system(_python_cmd(METRICS_SCRIPT, "-m " + scenes_string))
