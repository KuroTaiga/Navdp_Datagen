...
if TELESIM_GAUSSIAN_ROOT.exists() and str(TELESIM_GAUSSIAN_ROOT) not in sys.path:
    sys.path.append(str(TELESIM_GAUSSIAN_ROOT))
...
def main() -> int:
    ...
    if args.gaussian_model is not None:
        gaussian_model = args.gaussian_model
    else:
        preferred_plys = [
        ...
        gaussian_model = next((path for path in preferred_plys if path.exists()), preferred_plys[0])
