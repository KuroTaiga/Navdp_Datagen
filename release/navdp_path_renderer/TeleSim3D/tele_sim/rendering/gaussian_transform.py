...
class GaussianPly:
    """Structured representation of a Gaussian PLY buffer."""
    ...
    def read(cls, path: Path) -> "GaussianPly":
        ...
        if "vertex" not in ply:
            raise ValueError(f"PLY file {path} missing 'vertex' element")
        vertex = ply["vertex"]
    ...
    def write(self, data: np.ndarray, path: Path) -> None:
        ...
        ply = PlyData([new_element], text=self.ply.text)
...
def apply_transform_inplace(
    ...
    *,
    ...
    """Apply a 4x4 transform to positions (optionally normals/SH/quaternions/scales)."""
...
def concat_vertices(*arrays: np.ndarray) -> np.ndarray:
    """Concatenate structured vertex buffers along axis 0."""

    if not arrays:
        raise ValueError("No arrays provided for concatenation")
    dtype = arrays[0].dtype
    return np.concatenate([arr.astype(dtype, copy=False) for arr in arrays], axis=0)
