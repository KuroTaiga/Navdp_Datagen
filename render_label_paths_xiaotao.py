...
def offload_label_outputs(
    *, local_out_root: Path, nas_out_root: Path, scene_id: str, label_stem: str, verbose: bool = False
    ...
    - frames folder:   {local}/{scene}/{label_stem}/
    - video:           {local}/{scene}/{label_stem}.mp4
    - BEV image:       {local}/{scene}/{label_stem}_BEV.png
    ...
    # 1) frames directory (PNG sequence)
    ...
    # 2) mp4 video
    mp4 = local_scene_dir / f"{label_stem}.mp4"
    if mp4.exists():
        _safe_move(mp4, nas_scene_dir / mp4.name)
        moved.append((mp4, nas_scene_dir / mp4.name))
        if verbose: print(f"[OFFLOAD] moved video -> {nas_scene_dir/mp4.name}", flush=True)

    # 3) BEV debug image
    bev = local_scene_dir / f"{label_stem}_BEV.png"
    if bev.exists():
        _safe_move(bev, nas_scene_dir / bev.name)
        moved.append((bev, nas_scene_dir / bev.name))
        if verbose: print(f"[OFFLOAD] moved BEV -> {nas_scene_dir/bev.name}", flush=True)

    return moved

def maybe_offload_if_low_space(
    *, check_path: Path, min_free_bytes: int, local_out_root: Path, nas_out_root: Path,
    scene_id: str, label_stem: str, verbose: bool = False
) -> None:
    """
    ...
    if free_now < min_free_bytes:
        if verbose:
            ...
            print(f"[OFFLOAD] low free space (have {have} < need {need}); offloading {scene_id}/{label_stem} …", flush=True)
...
def consolidate_outputs_to_nas(
    *, local_out_root: Path, nas_out_root: Path, verbose: bool = False
    ...
    """
    Move all remaining outputs under local_out_root to nas_out_root, preserving:
      {root}/{scene}/{label_dir}/
      {root}/{scene}/{label}.mp4
      {root}/{scene}/{label}_BEV.png
    Skips known temp dirs like '__tmp_actor_combined'.
    """
    ...
    for scene_dir in sorted(local_out_root.iterdir()):
        ...
        # 1) Move label frame directories (exclude temp)
        ...
        # 2) Move files: .mp4 and *_BEV.png
        for entry in sorted(scene_dir.iterdir()):
            if entry.is_file() and (entry.suffix.lower() == ".mp4" or entry.name.endswith("_BEV.png")):
                dst = nas_scene_dir / entry.name
                try:
                    _safe_move(entry, dst)
                    if verbose:
...
def render_actor_follow_sequence(
    *,
    ...
    sh_degree: int,
    gpu_only: bool,
    follow_lag_points: int | None = None,
    # 新增 ↓↓↓
    meta: dict | None = None,
) -> tuple[list[np.ndarray], list[np.ndarray]]:
    """
    Render combined scene+actor frames.
    When follow_lag_points is not None, use index-based following:
      - camera starts at sampled point index 0
      - actor starts at index follow_lag_points
      - both advance one sampled point per frame (no extra interpolation)
      - stop when actor reaches the last sampled point
    Otherwise, fall back to original distance-based following.
    """
    ...
    # ---------------------------
    # A) Index-based follow mode
    # ---------------------------
    if follow_lag_points is not None:
        N = len(path_xy)
        if N < 2:
            return [], []

        lag = max(0, int(follow_lag_points))
        actor_start_idx = min(lag, N - 1)   # 演员起点（相机固定从0开始）
        camera_start_idx = 0
        ...
        frame_plans: list[ActorFramePlan] = []
        camera_positions: list[np.ndarray] = []
        actor_xy_seq: list[np.ndarray] = []

        # 相机/演员都沿采样点一步一格前进；演员到末尾后停止
        for a_idx in range(actor_start_idx, N):
            c_idx = max(camera_start_idx, a_idx - lag)
            # 相机位置（固定落在采样点上）
            c_xy = path_xy[c_idx]
            camera_positions.append(np.array([c_xy[0], c_xy[1], camera_z], dtype=np.float32))

            # 演员位置（同样落在采样点上）
            a_xy = path_xy[a_idx]
            actor_xy_seq.append(np.array(a_xy, dtype=np.float32))

            # 取演员朝向（基于邻域的平滑前向）
            actor_dir = forward_direction(path_xy, a_idx, window=STABILIZE_WINDOW if stabilize else 1)[:2]
            if np.linalg.norm(actor_dir) < 1e-6:
                actor_dir = np.array([0.0, 1.0], dtype=np.float32)

            theta = math.atan2(actor_dir[0], actor_dir[1]) + math.pi
            rotation_np = rotation_matrix_z_np(theta)
            translation_vec = np.array(
                [a_xy[0], a_xy[1], floor_z + options.foot_offset],
                dtype=np.float64,
            )
            ...
            # 动画帧选择
            if options.loop:
                base_idx = int(anim_cursor) % num_actor_frames
            else:
                base_idx = min(int(anim_cursor), num_actor_frames - 1)
            ...
            frame_plans.append(
                ActorFramePlan(
                    ...
                    transform=transform,
                    actor_pos_xy=np.array(a_xy, dtype=np.float32),
                    direction_xy=actor_dir.astype(np.float32),
                    camera_offset=0.0,
                )
            )

        # 汇总给 BEV / 返回
        camera_xy_seq = [p[:2].copy() for p in camera_positions]

        # ===== 接下来复用原有渲染循环（GPU/CPU 两种）=====
        frame_counter = 0
        prev_forward: np.ndarray | None = None
        direction_window = STABILIZE_WINDOW if stabilize else 1
        ...
        if gpu_only:
            ...
            for idx, plan in enumerate(frame_plans):
                ...
                # 相机朝向与目标点
                camera_position = camera_positions[idx]
                forward_vec = forward_direction(camera_positions, idx, window=direction_window)
                if np.linalg.norm(forward_vec[:2]) < EPS:
                    forward_vec = np.array([0.0, 1.0, 0.0], dtype=np.float32)
                if stabilize and prev_forward is not None:
                    blended = prev_forward * (1.0 - FORWARD_SMOOTH_BLEND) + forward_vec * FORWARD_SMOOTH_BLEND
                    bn = float(np.linalg.norm(blended))
                    if bn > EPS:
                        forward_vec = (blended / bn).astype(np.float32)
                        forward_vec[2] = 0.0
                prev_forward = forward_vec.copy()

                target_xy = camera_position[:2] - forward_vec[:2] * look_ahead
                target_z = max(camera_position[2] - look_down, floor_z + 0.05)
                target = np.array([target_xy[0], target_xy[1], target_z], dtype=np.float32)
                ...
                if video:
                    ...
                    imageio.imwrite(frame_path, render_uint8)
                        ...
                        # === 新增：为该帧写同名 JSON（只在 follow_lag_points 模式） ===
                    if follow_lag_points is not None and meta is not None:
                        cam_xy = np.array([camera_positions[idx][0], camera_positions[idx][1]], dtype=np.float32)
                        act_xy = np.array([actor_xy_seq[idx][0],      actor_xy_seq[idx][1]], dtype=np.float32)

                        u_cam, v_cam = _world_to_pixel(meta, cam_xy)
                        u_act, v_act = _world_to_pixel(meta, act_xy)

                        payload = {
                            "frame_index": int(frame_counter),
                            "raster_world": [
                                [float(cam_xy[0]), float(cam_xy[1])],
                                [float(act_xy[0]), float(act_xy[1])]
                            ],
                            "raster_pixel": [
                                [int(u_cam), int(v_cam)],
                                [int(u_act), int(v_act)]
                            ],
                            "camera_world": [float(camera_positions[idx][0]), float(camera_positions[idx][1]), float(camera_positions[idx][2])],
                            "actor_world":  [float(actor_xy_seq[idx][0]),     float(actor_xy_seq[idx][1]),     float(floor_z + options.foot_offset)],
                        }
                        with (frame_path.with_suffix(".json")).open("w", encoding="utf-8") as jf:
                ...
                if verbose and (idx % 10 == 0 or idx == len(frame_plans) - 1):
                    print(f"[VERBOSE] Scene {scene_id} / {label_id}: frame {idx + 1}/{len(frame_plans)} complete.", flush=True)

            if verbose:
                print(f"[VERBOSE] Scene {scene_id} / {label_id}: actor rendering complete ({frame_counter} frames).", flush=True)
    ...
    # ---------------------------------------
    # B) 兼容：原来的“按距离（米）跟随”逻辑
    # ---------------------------------------
    # ——下面保留你原先的实现（从这里开始到函数末尾），无需改动——
    # 原始实现从这里继续（sampler = PathSampler(path_xy) ...）
    sampler = PathSampler(path_xy)
    distances = list(sampler.cumulative)
    total_length = sampler.total_length
    scene_rest_dim = int(gaussians.get_features_rest.shape[1])
    ...
    if verbose:
        print(f"[VERBOSE] Scene {scene_id} / {label_id}: rendering actor path with {len(distances)} samples.", flush=True)
    ...
    # 下面保持你原有渲染实现（GPU/CPU 两个分支）——与上面的 index-based 分支一致，这里不再重复
    # ...（原代码保持不变）
            ...
            if actor_runtime is not None:
                ...
                if hide_actor_enabled:
                    ...
                    cam_seq, act_seq = render_actor_follow_sequence(
                        ...
                        gpu_only=gpu_only,
                        follow_lag_points=follow_lag_points,
                        meta=meta,  # ← 新增
                    )
                ...
                if render_bev:
                    ...
                    save_bev_debug_image(
                        ...
                        out_path=bev_path,
                        look_ahead=look_ahead,
                        follow_points=follow_lag_points,
...
def parse_args() -> ArgumentParser:
    ...
    type=int,
    default=None,
    help="Index-based follow: actor starts at this many points ahead of the camera (camera fixed at index 0). "
         "If set, overrides --follow-distance and ends when the actor hits the last point."
    )
...
def main() -> None:
    ...
    follow_lag_points = None if args.follow_lag_points is None else int(args.follow_lag_points)
    ...
    try:
        for scene_idx, scene_id in enumerate(scene_ids, start=1):
            ...
            for json_path in json_files:
                ...
                if follow_lag_points is not None:
                    est_frames = max(0, len(prepared.path_xy) - int(follow_lag_points))
                else:
            ...
            if nas_offload_dir is not None:
                if verbose_enabled:
                    print("[CONSOLIDATE] Final pass: moving any remaining outputs to NAS …", flush=True)
